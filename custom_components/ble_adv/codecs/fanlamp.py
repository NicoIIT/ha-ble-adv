...
class FanLampEncoderV1m0(FanLampEncoderV1Base):
    """FanLamp V1 Multi R0 encoder."""
...
class FanLampEncoderV1m1(FanLampEncoderV1m0):
    """FanLamp V1 Multi R1 encoder."""

    def convert_multi_from_enc(self, enc_cmd: BleAdvEncCmd, conf: BleAdvConfig) -> list[bytes]:
        """Convert an encoder command and a config into a list of readable buffers."""
        uid = (conf.id | (conf.index >> 8)).to_bytes(2, "little")
        return [
            bytes([enc_cmd.cmd, *uid, enc_cmd.arg0, enc_cmd.arg1, enc_cmd.arg2, i, enc_cmd.param, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
            for i in [0x01, 0x02, 0x03, 0x04]
        ]
...
def _get_cww_translators(param_attr: str, cold_attr: str, warm_attr: str) -> list[Trans]:
    return [
        ...
        Trans(CTLightCmd().act(ATTR_CMD, ATTR_CMD_BR_UP).eq(ATTR_STEP, 0.1), EncCmd(0x04).eq("arg2", 0x80)).no_direct(),
        Trans(CTLightCmd().act(ATTR_CMD, ATTR_CMD_BR_DOWN).eq(ATTR_STEP, 0.1), EncCmd(0x05).eq("arg2", 0x80)).no_direct(),
        Trans(CTLightCmd().act(ATTR_CMD, ATTR_CMD_CT_DOWN).eq(ATTR_STEP, 0.1), EncCmd(0x06).eq("arg2", 0x80)).no_direct(),
        Trans(CTLightCmd().act(ATTR_CMD, ATTR_CMD_CT_UP).eq(ATTR_STEP, 0.1), EncCmd(0x07).eq("arg2", 0x80)).no_direct(),
        Trans(
...
FLCODECS = [
    ...
    FanLampEncoderV1m0().id(FLV1, "r0").header([0xF0, 0xFF]).ble(0x19,0xFF).add_rev_only_trans(TRANS_FANLAMP_V1),
    FanLampEncoderV1m1().id(FLV1, "r1").forced_crc2(0x00).header([0xF0, 0xFF]).ble(0x19,0xFF).add_rev_only_trans(TRANS_FANLAMP_V1),

...
BASE_TECH_CONF = {CONF_ADAPTER_ID: "adapter_id", CONF_REPEAT: 1, CONF_INTERVAL: 20, CONF_DURATION: 100}
...
async def test_migrate_v1_no_adapter(hass: HomeAssistant) -> None:
    ...
    conf = await create_entry(hass, "idv10", 1, BASE_CONF)
    await async_migrate_entry(hass, conf)
    assert conf.data[CONF_TECHNICAL][CONF_ADAPTER_ID] == "adapter_id"


async def test_migrate_v1_one_adapter(hass: HomeAssistant) -> None:
    """Test migration from config v1."""
    conf = await create_entry(hass, "idv11", 1, BASE_CONF)
    coordinator = await get_coordinator(hass)
    coordinator.get_adapter_ids = mock.MagicMock(return_value=["new_id"])
    await async_migrate_entry(hass, conf)
    assert conf.data[CONF_TECHNICAL][CONF_ADAPTER_ID] == "new_id"


async def test_migrate_v1_two_adapter(hass: HomeAssistant) -> None:
    """Test migration from config v1."""
    conf = await create_entry(hass, "idv12", 1, BASE_CONF)
    coordinator = await get_coordinator(hass)
    coordinator.get_adapter_ids = mock.MagicMock(return_value=["new_id2", "other_id"])
    await async_migrate_entry(hass, conf)
    assert conf.data[CONF_TECHNICAL][CONF_ADAPTER_ID] == "new_id2"


async def test_migrate_v2(hass: HomeAssistant) -> None:
    """Test migration from config v2."""
    conf_wrong_adapt = BASE_CONF.copy()
    conf_wrong_adapt[CONF_TECHNICAL].pop(CONF_ADAPTER_ID)
    ...
    await async_migrate_entry(hass, conf)
    assert conf.data[CONF_TECHNICAL][CONF_ADAPTER_ID] == "prev_adapter_id"
